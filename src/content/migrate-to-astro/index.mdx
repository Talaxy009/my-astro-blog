...
更新：最近想让 Astro 来处理推文图像，其 Image 组件能自动把远程图像拉下来，把图像源变为本站就不会出现部分读者因无法访问推特而看不见图了。没法在 reamrk 插件上实现该功能，于是又写了一个 Astro 组件，在 MDX 内使用就能实现一样的效果，且可覆盖的场景更多（为啥我最初没想到这样做呢）。不管怎么说，这是那个 Astro 组件：

[$card](https://astro-tweet-card.talaxy.site)
